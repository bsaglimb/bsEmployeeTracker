...
// Add methods to the class
    ...
    getEmployeeByMgrId(data){
        ...
        .promise()
        .query(
            'SELECT e.first_name AS "first name", e.last_name AS "last name", CONCAT(mgmt.first_name, " ", mgmt.last_name) AS Manager FROM employee e INNER JOIN employee mgmt ON e manager_id = mgmt.id WHERE e.manager_id = ?', values);
    }
    ...
    getBudgetByDepartment(data){
        return this.db
            .promise()
            .query(
                'SELECT d.department_name AS department, SUM(r.salary) AS budget FROM role r INNER JOIN department d ON r.department_id = d.id GROUP BY department',);
    }
     
    getRoles(){
        return this.db
        .promise()
        .query('SELECT r.titles AS title, r.salary, d.department_name AS department FROM role r LEFT JOIN department d ON r.department_id = d.id ORDER BY department, r.id ASC');
    }
    ...
    getManagers(){
        return this.db
        .promise()
        .query('SELECT id, CONCAT(first_name, " ", last_name) AS manager_name FROM employee WHERE manager_id IS NULL');
    }
