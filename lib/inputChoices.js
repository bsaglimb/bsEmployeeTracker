const sql = require('../db/query_lib.js') ;
...
const deptChoices = async () => {
    const tempArr = await sql.getDepartmentsByIds();
...
const mgmtChoices = async () => {
    const tempArr = await sql.mgmtChoices();
    const choices = tempArr[0];
...
const nonMgmtChoices = async () => {
    const tempArr = await sql.nonMgmtChoices();
    const choices = tempArr[0];
